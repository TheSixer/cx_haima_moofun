...
Page({
  ...
  changeEngine:function (e) {   //发动机号码
    this.setData({
      engine: e.detail.value
    })
  },
  vinChange: function (e) {   //车辆识别码
    this.setData({
      vin: e.detail.value
    })
  },
    ...
    http._get(url, data, 
      function(res) {
        ...
        dealErr.dealErr(res, function() {
          ...
          if(res.data.data.driverLicense) {
            var data = res.data.data.driverLicense
            console.log(data)
            that.setData({
  ...
  submitRemind: function () {
    ...
    var url = that.data.APIUrl + '/moofun/remind/drivingLicence/miniApps' + token,
        data = {
          uid: that.data.uid,
          carId: that.data.carId,
    ...
    if(that.data.requestTypePost) {
      ...
      url = that.data.APIUrl + '/moofun/remind/drivingLicence/miniApps/' + that.data.id + token  //修改需要id
